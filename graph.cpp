...
auto Graph<Trait>::get_edgesVector() {
    vector<Edge *> visited;
...
template<class Trait>
unsigned long Graph<Trait>::size() {
    nodes_vector.size();
}

template<class Trait>
unsigned long Graph<Trait>::num_edges() {
    vector<Edge *> visited;
    for (Node *node : nodes_vector) {
        for (Edge *e : node->getEdges_list())
            if (find(visited.begin(), visited.end(), e) != visited.end())
...
template<class Trait>
auto Graph<Trait>::find_edge(Node *start, Node *end, EdgeContent content) {
...
template<class Trait>
auto Graph<Trait>::find_node(NodeContent content) {
...
template<class Trait>
bool Graph<Trait>::insert_node(NodeContent node_content) {
    Node *node = new Node();
...
template<class Trait>
void Graph<Trait>::insert_edge(EdgeContent edge_content, NodeContent start, NodeContent end, bool is_directed) {
    Edge *edge = new Edge();
...
template<class Trait>
bool Graph<Trait>::delete_node(NodeContent content) {
...
template<class Trait>
bool Graph<Trait>::delete_edge(NodeContent start, NodeContent end, EdgeContent content) {
...
template<class Trait>
auto Graph<Trait>::bfs(NodeContent content) {
    ...
    while (!node_queue.empty()) {
        ...
        if (std::find(discovered.begin(), discovered.end(), current) == discovered.end()) {
            ...
            for (Edge *edge : current->getEdges_list()) {
                ...
                if (to_be_added ==
                    nullptr) //|| std::find(discovered.begin(), discovered.end(), to_be_added) == discovered.end())
                    continue;
...
template<class Trait>
auto Graph<Trait>::dfs(NodeContent content) {
...
template<class Trait>
void Graph<Trait>::describe_helper(NodesVector to_describe) {
...
/* template<class Trait>
 NodesVector Graph<Trait>::dijkstra(NodeContent start) {
